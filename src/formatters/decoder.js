...
class Decoder {
  static toHydraAddress(hexAddress, isMainnet = true) {
    ...
    let lAddress = hexAddress;
    // Add network byte
    if (isMainnet) {
      lAddress = MainnetNetworkByte + lAddress;
    } else {
      lAddress = TestnetNetworkByte + lAddress;
    }

    const lAddressBuffer = Buffer.from(lAddress, 'hex');
    // Double SHA256 hash
    const hash1 = crypto.createHash('sha256').update(lAddressBuffer).digest('Hex');
    const hash1Buffer = Buffer.from(hash1, 'hex');
    const hash2 = crypto.createHash('sha256').update(hash1Buffer).digest('Hex');

    // get first 4 bytes
    lAddress += hash2.slice(0, 8);

    // base58 encode
    const address = bs58.encode(Buffer.from(lAddress, 'hex'));
    return address;
  }
  ...
  static mapToBN(el) {
    if (el.type === 'BigNumber') {
      ...
      Object.keys(el).forEach((key) => {
        el[key] = this.mapToBN(el[key]); // eslint-disable-line no-param-reassign
      });
    }
  ...
  static decodeCall(rawOutput, contractABI, methodName, removeHexPrefix = true) {
    ...
    if (methodABI && 'executionResult' in output && 'output' in output.executionResult) {
      ...
      let formattedOutput = JSON.parse(JSON.stringify(iface.decodeFunctionResult(methodName, Utils.appendHexPrefix(output.executionResult.output))));
